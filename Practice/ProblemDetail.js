...
const ProblemDetail = ({ route, navigation }) => {
  ...
  // 북마크 가져오기
  useEffect(() => {
    const bookmarksRef = ref(database, 'bookmark/' + examDoc.id + '/' + userId);
    const listener = onValue(bookmarksRef, (snapshot) => {
      const data = snapshot.val();
      if (data && data.bookmarks) {
        setBookmarks(data.bookmarks);
      } else {
        // 데이터가 없는 경우 상태를 초기화합니다.
        setBookmarks([]);
      }
    });

    return () => {
      off(bookmarksRef, listener);
    };
  }, [userId]);
  ...
  const bookmarkSave = () => {
    ...
    const postRef = ref(database, 'bookmark/' + examDoc.id + '/' + userId);
